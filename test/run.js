#!/usr/bin/env node
// End-to-end test: runs the tool against test/fixtures and compares every
// output file against the golden files in test/output/.
...
const GOLDEN = path.join(__dirname, 'output');
const TMP = path.join(__dirname, '.tmp');

// CLI args that reproduce the golden output
const TOOL_ARGS = [
	'app.js',
	'--input', 'test/fixtures',
	'--output', 'test/.tmp',
	...
	'--output-format', 'auto',
	'--save-images', 'none',
	'--frontmatter-fields', 'title,date,permalink,status,categories,tags,author,authorSlug,slug,type,excerpt,taxonomies',
	'--terms-as-objects', 'true',
	'--plugins', 'acf,yoast,rankmath,woocommerce',
	'--rewrite-links', 'true',
	'--emit-redirects', 'true',
	'--emit-taxonomies', 'true',
	'--emit-authors', 'true',
	'--post-folders', 'true',
	'--write-delay', '0',
];
...
function checkReport(label, golden, actual) {
	try {
		...
		// Strip run-specific dynamic fields before comparing
		for (const o of [g, a]) {
			delete o.startedAt;
			delete o.finishedAt;
			delete o.output;
...
function checkReportTxt(label, golden, actual) {
	// Strip dynamic lines (timestamps + output path differ between runs)
	const SKIP = /^(Started:|Finished:|Output:)/;
	const filter = (s) => normalize(s).split('\n').filter((l) => !SKIP.test(l.trim())).join('\n');
	if (filter(golden) === filter(actual)) {
		pass(label + ' (timestamps/output excluded)');
	} else {
		fail(label + ' (timestamps/output excluded)', 'text content differs');
	}
}

function checkRedirects(label, golden, actual) {
	// Redirect line order may vary; sort before comparing
...
function check(goldenPath) {
	...
	if (!fs.existsSync(actualPath)) {
		fail(label, 'file missing in actual output');
		return;
	}
...
console.log(`${GRAY}Running tool...${RESET}`);
try {
